...
class CreateModulesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create(config('admin.tables.modules'), function (Blueprint $table) {
            $table->bigIncrements('id');
            $table->string('title', 50);
            $table->integer('page_id')->default(0)->comment('页面ID');
            $table->string('page_code', 10)->default('')->comment('页面标识');
    ...
    {
        Schema::dropIfExists(config('admin.tables.modules'));
    }
}
